...
정식 API를 연결하기 전의 개발과 내부 시연이 주 사용 범위입니다. 제3자를 위한
서비스에 구독 모델을 사용하는 것은 약관상 권장되지 않습니다. 정식 운영에서는 각
공급자가 지원하는 API 인증 방식으로 전환해야 합니다.
...
제3자를 위한 서비스에 Claude 구독 모델을 사용하는 것은 약관상 권장되지 않습니다.
정식 운영 전에는 Anthropic이 지원하는 API 인증으로 전환해야 합니다.
